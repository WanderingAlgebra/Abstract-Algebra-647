...
域是抽象代数中条件最严格的代数结构：它要求乘法有单位元和逆元，这比环和群的要求都强。
正因如此，域的例子虽然比环和群少，但结构最为清晰。
...
这个例子将展示如何从一个无理数构造出一个域，同时也会暴露逐条验证域公理的局限性，从而引出更一般的构造方法。
...
看起来由两个根生成的域，实际上也可能由一个元素生成；同时，两个根之间的乘积 $\sqrt{6}$ 会被带进来，说明构造域时不能只看"添加了几个元素"，还要看这些元素之间的代数关系。
...
这个反例揭示了域同态存在性的一个深刻约束：代数元在同态下的像必须满足同一个多项式方程。
例如 $\sqrt{2}$ 满足 $x^2 - 2 = 0$，所以它的像也必须满足这个方程。
因此，域同态的存在性取决于目标域中是否包含相关多项式的根。
...
\paragraph*{例 2：$\mathbb{Q}(\sqrt{2}+\sqrt{3})$}
乍看之下，$\sqrt{2}+\sqrt{3}$ 是一个很"混合"的生成元：它把两个根加在一起，好像把独立的信息混在了一起。
...
说 $E/F$ 不是有限生成扩张，并非仅仅意味着"生成元有无限多个"，而是说：无论从 $E$ 中挑出哪有限多个元素，它们都生成不出整个 $E$。
...
而 Galois 群中的自同构又不能随便移动基元素，只能把每个基元素映到它的极小多项式在 $E$ 中的根。
